...
- **Fáze:** Produkce (**https://michal-varchol-web.vercel.app**, deploy 2026-08-28, commit `9ad7da5`, Vercel deploy `dpl_DCgQNrTqrENjkQU39anNLQ4A3hoW` READY) běží s antracit+teak paletou, hero fotem a upravenou sekcí Reference (bez úvodní věty o Firmy.cz). **Lokálně, ještě NEnasazeno:** zesvětlený `.hero__scrim` overlay (viz níže) — na produkci je pořád tmavší verze. Kontaktní formulář na produkci STÁLE NEfunguje (chybí `RESEND_API_KEY` na Vercelu), některá klientská fakta pořád chybí (rok založení, logo, DIČ, skutečná doména). **Klient Michal Varchol už produkci VIDĚL** (dal zpětnou vazbu na hero foto 2026-08-30) — ale formálně neschválil paletu, která přepsala jeho dříve zamčenou hnědou.
- **Poslední session:** 2026-08-30 — klient (přes uživatele) nahlásil, že hero fotka na produkci působí vybledlá/tmavá. Zdroj: `.hero__scrim` gradient v `assets/css/style.css` (~L409), ne filtr na obrázku. Alfa snížena z `.15/.72` na `.05/.5` (dolní hodnota ponechána nenulová kvůli kontrastu bílého textu/tlačítek). Ověřeno Playwright screenshotem, beze změny nasazeno lokálně — čeká na commit+push. Viz `memory.md` záznam "Zesvětlení hero fota (zpětná vazba od klienta)".
...
- **Známé vědomé mezery:** **`RESEND_API_KEY` není nastaven na Vercelu — kontaktní formulář na živé produkci aktuálně neodešle e-mail** (nejvyšší priorita), zesvětlený hero scrim je zatím jen lokální/necommitnutý (nutno nasadit), canonical/OG URL neodpovídá skutečné nasazené adrese, drobný horizontální overflow na ~360px u `.reference-cta`/`.services-grid__item--lead`, alt texty "others" fotek v galerii jsou generické vzorce, tap targety pod 40px na mobilním menu/patičce/desktop navu, `node_modules` nenainstalované, mapa používá Google Maps embed místo mapy.cz, zbytek a11y (screen reader) ještě neprošel manuálním testem.
- **Další logický krok:** commitnout a nasadit zesvětlený hero scrim (pokud uživatel/klient potvrdí, že je to ono), založit Resend účet a nastavit `RESEND_API_KEY` na Vercelu, opravit drobný mobilní overflow, potvrdit canonical/OG URL vs. skutečnou doménu, využít klientovu aktivní pozornost a získat od něj zbývající otevřené informace + formální schválení palety.
