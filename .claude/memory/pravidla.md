...
- **Primární barva:** antracitová (`--primary:#2E3237`, tmavší `--primary-dark:#17191C`) — nahradila dřívější dřevěnou hnědou 2026-08-27 na přímý pokyn uživatele (vývojář/agentura, ne klient přímo). **Pozor:** dřívější hnědá paleta byla 2026-08-24 klientem explicitně zamčená na základě jím dodané referenční fotky luxusního dřevěného interiéru — tato antracitová varianta danou fotoreferenci nerespektuje a **klient ji zatím neviděl ani neschválil**, stejně jako zbytek aktuálně živé produkce (viz `index.md`). **Petrolová/modro-zelená varianta (`#3F5A54`/`#2E4440`) byla klientem dříve výslovně zamítnuta** ("nechce takovou tu modro zelenou") — antracitová je barevně odlišná (neutrální tmavá šedá, ne modro-zelená), ale při další komunikaci s klientem stojí za to si zamítnutí petrolové připomenout, kdyby antracitová vyvolala podobnou námitku.
- **Pozadí:** beze změny — teplá krémová (`--bg:#FAF6EF`, `--bg-alt:#F1E9DB`), klient si dříve výslovně přál "světlejší web". Ne čistě bílá. Uživatel explicitně zvolil variantu "antracit jako primární, světlé pozadí zůstává" (ne plně tmavý motiv).
- **Text:** antracitová (`--fg:#24272B`, dřív tmavá ořechová hnědá), neutrální tlumená šedá pro sekundární text (`--muted:#5C5F63`, dřív teplá hnědá) — ne čistě černá.
- **Accent:** teakové dřevo — středně tmavý teak (`--accent:#8B5E3C`) a světlejší písčitý teak (`--accent-2:#C39A66`, dřív olivově-taupe). Poměr cca 80-85 % neutrální plocha / zbytek accent, accent nesmí zabírat velké plochy.
- Kontrast text/pozadí min. WCAG AA pro každou kombinaci — u antracit+teak palety orientačně ověřeno výpočtem (`--accent` na krémovém pozadí ~5:1, `--muted` na krémovém pozadí ~5,8:1) a vizuálně přes headless Playwright screenshoty (hero, o mně, služby, galerie, reference, kontakt, patička) bez console chyb. `--error-text:#B23B1F` zůstal beze změny, framed jako samostatná sémantická barva mimo brand paletu.
...
- Paleta byla 2026-08-24 poprvé zamčená na hnědou podle klientovy reference; 2026-08-27 přebarvena na antracit+teak podle přímého pokynu uživatele (viz `assets/css/style.css` `:root`) — tato novější verze **ještě nebyla klientem schválena** a měla by se mu ukázat spolu se zbytkem nasazené produkce.
